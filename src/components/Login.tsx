...
// Definimos la interfaz para que TypeScript no de errores
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user.password === password) {
        // Guardamos sesión local para evitar cierres inesperados
        localStorage.setItem('pos_current_user', JSON.stringify(user));
        
        // EJECUTAMOS LA FUNCIÓN DE ÉXITO (Esto es lo que daba error antes)
        if (onLoginSuccess && typeof onLoginSuccess === 'function') {
          onLoginSuccess(user);
        } else {
          console.error("Error crítico: onLoginSuccess no es una función");
        }
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="p-8 text-center bg-gray-50 border-b">
          ...
          <h1 className="text-2xl font-black text-gray-800 uppercase tracking-tight">Cafetín Alí</h1>
          <p className="text-gray-500 text-sm">Inicia sesión para continuar</p>
        </div>

        <form onSubmit={handleLogin} className="p-8 space-y-6">
          {error && (
            <div className="p-3 bg-red-50 text-red-700 rounded-xl text-xs font-bold border border-red-100 animate-pulse text-center">
              {error}
            </div>
          )}
